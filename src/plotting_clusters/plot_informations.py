...
def show_audio(filename):
    ...
    # plot the spectgram
    _, _, t, im = axarr[1].specgram(signal, NFFT=1024, Fs=sample_rate,
...
def show_audio_with_cluster(signal, sample_rate, cluster, show_signal=True, show_spectrogram=True):
    ...
    _, axarr = plt.subplots(total_subplots, sharex=True)
    ...
    if show_spectrogram:
        # plot the spectgram
        _, _, _, im = axarr[id_subplot].specgram(
            signal, NFFT=1024, Fs=sample_rate, noverlap=512,
    ...
    axarr[id_subplot].matshow(m, aspect='auto', origin='lower', extent=[0, len(signal) / sample_rate, 0, dim_x], cmap=cmap)
...
def vector_of_cluster_to_matrix(vector, number_max=None):
    '''
    :param vector: should contains values >= 0
    :param number_max: the number of clusters. If it is equal to None the
                       function will find the adequat value from vector
    ...
    for i in range(len(vector)):
        index = values_possible.index(vector[i]) if number_max == None else vector[i]
...
if __name__ == '__main__':
    ...
    vector[0:len(signal) / 2] = 1
    vector[len(signal) / 2:len(signal) * 3 / 4] = 2

    clusters = vector_of_cluster_to_matrix(vector)

    show_audio_with_cluster(signal, sample_rate, clusters)
