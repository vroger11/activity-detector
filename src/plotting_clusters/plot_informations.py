...
def show_audio_with_cluster(signal, fs, cluster, show_signal=True, show_spectrogram=True):
    ...
    f, axarr = plt.subplots(total_subplots, sharex=True)
    ...
    if show_spectrogram:
        # plot the spectgram
        spectrum, freqs, t, im = axarr[id_subplot].specgram(signal, NFFT=1024, Fs=fs, noverlap=512,
...
def vector_of_cluster_to_matrix(vec, number_max=None):
    ...
    :param number_max: the number of clusters. If it is equal to None the function will find the adequat value from vec
